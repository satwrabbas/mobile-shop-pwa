...
import { updateProductQuickDetails, deleteProductAction } from "../products/actions";
import Image from "next/image";
import { Edit, Trash2, X, Check, Loader2 } from "lucide-react";

export default function AdminProductsList({ initialProducts }: { initialProducts: any[] }) {
  const [products, setProducts] = useState(initialProducts);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [loadingId, setLoadingId] = useState<string | null>(null);

  // حالة النماذج المؤقتة للتعديل
  const [editForm, setEditForm] = useState({ price: 0, discount_price: "", stock: 0 });
      ...
      {products.map((product) => (
        <div key={product.id} className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex flex-col md:flex-row p-4 gap-4 items-center">
          ...
          <div className="relative w-24 h-24 bg-gray-50 rounded-xl overflow-hidden shrink-0">
            <Image src={product.image_urls?.[0] || 'https://placehold.co/100'} alt={product.title} fill className="object-cover" />
          </div>
          ...
          <div className="flex-grow text-center md:text-right w-full">
            ...
            {editingId === product.id ? (
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm font-medium">
                <span className="text-gray-900">السعر: {product.price} د.ك</span>
                {product.discount_price && <span className="text-red-600">التخفيض: {product.discount_price} د.ك</span>}
