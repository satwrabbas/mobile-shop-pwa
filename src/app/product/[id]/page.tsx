          ...
          <div className="flex flex-col">
            ...
            <div className="mb-6 pb-6 border-b border-gray-100">
              {hasDiscount ? (
                <div className="flex items-end gap-3">
                  <span className="text-4xl font-bold text-red-600">{product.discount_price} د.ك</span>
                  <span className="text-lg text-gray-400 line-through mb-1">{product.price} د.ك</span>
                </div>
              ) : (
                <span className="text-4xl font-bold text-gray-900">{product.price} د.ك</span>
              )}
            </div>
            ...
            {product.specs && (
              <div className="mb-8 bg-gray-50 p-4 rounded-xl">
                <h3 className="font-semibold text-gray-900 mb-3">المواصفات الرئيسية:</h3>
                <ul className="space-y-2">
                  {Object.entries(product.specs).map(([key, value]) => (
                    <li key={key} className="flex items-center text-gray-600 text-sm">
                      <span className="w-24 font-medium text-gray-800 capitalize">{key}:</span>
